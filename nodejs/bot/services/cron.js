...
let notice = cron.schedule('* * * * *', () => {
    ...
    // const [year, month, day, week, hour, minutes] = [2023, 11, 16, 4, 15, 0];
    ...
    for (const guildKey in fetchData.entryGuilds) {
        ...
        for (const kadaiKey in fetchData.kadai[guildId]) {
            ...
            /* 当日の課題締め切り時刻になったらStateをfalseに */
            if (year == dl_year && month == dl_month && day == dl_day && hour == dl_hour && minutes == dl_minutes) {
                firestore.db.collection(`data/channels/${kadaiData['guildId']}/`).where('subject', '==', kadaiData['subject']).get()
                    .then(async (res) => {
                        firestore.db.collection(`notice/kadai/${kadaiData['guildId']}/`).where('entry_date', '==', kadaiData['entry_date']).get()
                            .then(async (res) => {
                                firestore.db.collection(`notice/kadai/${kadaiData['guildId']}/`).doc(`${res.docs[0].id}`).update({ 'state': false })
                            })
                    });
            }
            ...
            if (year == dl_year && month == dl_month && day == dl_day && hour == dl_hour && minutes == dl_minutes) {
                ...
                firestore.db.collection(`data/channels/${remindData['guildId']}/`).where('subject', '==', remindData['subject']).get()
                    .then(async (res) => {
                        const channelId = res.docs[0].data()['channel_id'];
                        messageController.send({ contents: text, channel: channelId })
                        firestore.db.collection(`notice/remind/${remindData['guildId']}/`).where('entry_date', '==', remindData['entry_date']).get()
                            .then(async (res) => {
                                firestore.db.collection(`notice/remind/${remindData['guildId']}/`).doc(`${res.docs[0].id}`).update({ 'state': false })
                            })
                    });
            }
        }
